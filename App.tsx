...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen pb-20 bg-[#f4f7f6]">
      {/* Header / Hero Section */}
      <header className="relative h-[500px] flex items-center justify-center overflow-hidden bg-[#2c5f69]">
        ...
        <div className="container mx-auto px-4 relative z-10 text-center text-white">
            ...
            <div className="w-48 h-48 rounded-full border-4 border-white shadow-2xl mx-auto overflow-hidden bg-slate-300">
              <img 
                ...
                onError={(e) => {
                  ...
                  // Se falhar o carregamento da imagem do GitHub, mostra um avatar com as iniciais
          ...
          <h1 className="text-4xl md:text-5xl font-extrabold mb-2 tracking-tight drop-shadow-md">
            {personalInfo.name}
          </h1>
          <p className="text-xl text-teal-50 font-medium mb-6 max-w-2xl mx-auto drop-shadow-sm">{personalInfo.title}</p>
          
          <div className="flex flex-wrap justify-center gap-4 text-sm mb-6">
            <a href={`mailto:${personalInfo.email}`} className="flex items-center gap-2 bg-black/20 hover:bg-black/40 px-4 py-2 rounded-full transition-all backdrop-blur-sm">
            ...
            {personalInfo.phones.map(phone => (
              <a key={phone} href={`tel:${phone.replace(/\D/g,'')}`} className="flex items-center gap-2 bg-black/20 hover:bg-black/40 px-4 py-2 rounded-full transition-all backdrop-blur-sm">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
          ...
          <div className="flex justify-center gap-4">
            <a href={personalInfo.social.linkedin} target="_blank" rel="noopener noreferrer" className="bg-white text-[#0077b5] hover:scale-110 p-2 rounded-full transition-all shadow-lg">
              <svg className="w-6 h-6 fill-current" viewBox="0 0 24 24"><path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/></svg>
            </a>
            <a href={personalInfo.social.facebook} target="_blank" rel="noopener noreferrer" className="bg-white text-[#1877f2] hover:scale-110 p-2 rounded-full transition-all shadow-lg">
